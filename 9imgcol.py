...
#Convert Alpha to Free Channel
    alpha = alpha[:, :, None].astype(np.float32)
#composite Foregroundand background using alpha matte
    return (alpha * foreground + (1 - alpha) * background).astype(np.uint8)
    #alpha = np.stack((alpha,alpha,alpha), axis=2)                    #miss 
    #composited_image = alpha * foreground + (1 - alpha) * background  #miss 
    #return composited_image                                           #miss 

#Example Usage
if __name__ == "__main__":
#Read foreground,background and trimap images
    foreground = cv2.imread(r"C:\CV\model.jpg")
    background = cv2.imread(r"C:\CV\model.jpg")
    trimap=cv2.imread(r"C:\CV\model.jpg", cv2.IMREAD_GRAYSCALE)
